...
from data_utils import get_data_loaders, load_caltech101_data, visualize_batch
...
def main():
    ...
    for inputs, labels in train_loader:
        visualize_batch(inputs, labels, class_names)
        break
    
    if args.mode == 'finetune':
        # 微调模型
        print("微调AlexNet模型...")
        model = create_alexnet_model(num_classes=len(class_names), pretrained=True)
        model = model.to(device)
        ...
        # 从头训练模型
        print("从头训练AlexNet模型...")
        model = create_alexnet_model(num_classes=len(class_names), pretrained=False)
        model = model.to(device)
        ...
        model = create_alexnet_model(num_classes=len(class_names))
        ...
        # 加载微调模型
        finetune_model = create_alexnet_model(num_classes=len(class_names))
        finetune_model = load_model(finetune_model, args.model_path, device)
        finetune_model = finetune_model.to(device)
        
        # 加载从头训练模型
        scratch_model = create_alexnet_model(num_classes=len(class_names))
        ...
        print(f"微调模型准确率: {acc:.4f}, 从头训练模型准确率: {acc:.4f}")
        print(f"准确率提升: {acc - acc:.4f}")
